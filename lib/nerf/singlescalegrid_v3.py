...
class Sine(nn.Module):
    def __init__(self, w0=30.):
        ...
        self.w0 = torch.nn.Parameter(torch.tensor([w0]))
        self.phase_shift = torch.nn.Parameter(torch.tensor([0.]))
        # self.w0 = w0

    def forward(self, x):
        return torch.sin(self.w0 * x + self.phase_shift)
...
class SirenDecoder(nn.Module):
    def __init__(self, latent_dim, W=4, rgb_latent_dim=15):
        super(SirenDecoder, self).__init__()
        first_layer_w0 = 25.
        following_layers_w0 = 1.
        self.positional_mlp = nn.Sequential(
            SirenLayer(input_dim=latent_dim, hidden_dim=W, w0=first_layer_w0),
            SirenLayer(input_dim=W, hidden_dim=W, w0=following_layers_w0),
            nn.Linear(in_features=W, out_features=1 + rgb_latent_dim)
        )
        ...
        self.directional_mlp = nn.Sequential(
            SirenLayer(input_dim=dim0, hidden_dim=W, w0=following_layers_w0),
            SirenLayer(input_dim=W, hidden_dim=W, w0=following_layers_w0),
            SirenLayer(input_dim=W, hidden_dim=W, w0=following_layers_w0),
            nn.Linear(in_features=W, out_features=3)
        )
...
class PriorModules(nn.Module):
    def __init__(self, latent_dim, num_modules):
        ...
        self.module_list = nn.ModuleList([SirenDecoder(latent_dim=latent_dim) for i in range(num_modules)])
    ...
    def forward(self, xyz, viewdirs, latents):
        ...
        # final_output = self.module_list[3].forward(latents, viewdirs)
...
class SingleScaleGrid(nn.Module):
    def __init__(self,
        ...
        self.prior_modules = PriorModules(latent_dim=latent_dim, num_modules=16)
    ...
    def extract_latent(self, xyz, viewdirs):
        ...
        output = self.prior_modules.forward(ind_norm.squeeze(), viewdirs, latent)
    ...
    def forward(self, rays_o, rays_d, viewdirs, global_step=None, **render_kwargs):
        ...
        density, raw_rgb = self.extract_latent(ray_pts[~mask_outbox], viewdirs[~mask_outbox])
