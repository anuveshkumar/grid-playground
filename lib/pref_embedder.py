...
class PREF(nn.Module):
    def __init__(self, linear_freqs=[256, 256, 256],
                 reduced_freqs=[1] * 3, feature_dim=16, sampling='linear',
                 device='cuda',
                 **kwargs) -> None:
        """
        ...
        Nx, Ny, Nz = linear_freqs
        d1, d2, d3 = reduced_freqs
        self.params = nn.ParameterList(
            [nn.Parameter(torch.zeros(1, feature_dim, d1, Ny, Nz).to(torch.complex64).to(self.device)),
             nn.Parameter(torch.zeros(1, feature_dim, Nx, d2, Nz).to(torch.complex64).to(self.device)),
             nn.Parameter(torch.zeros(1, feature_dim, Nx, Ny, d3).to(torch.complex64).to(self.device))]
        )
    ...
    def forward(self, inputs, bound=1.0):
        # map to [-1, 1]
        inputs = inputs / bound
    ...
    def compute_fft(self, features, xyz_sampled, interp=True):
        if interp:
            ...
            Fx, Fy, Fz = [feature for feature in features]
            d1, d2, d3 = Fx.shape[2], Fy.shape[3], Fz.shape[4]
            kx, ky, kz = self.axis
            kx, ky, kz = kx[:d1], ky[:d2], kz[:d3]
...
if __name__ == "__main__":
    encoder = PREF(feature_dim=16, sampling='log')
    x = torch.FloatTensor(1000, 3).uniform_(-1.4, 1.4).cuda()
    out = encoder.forward(x, bound=1.4)
    loss = encoder.parseval_loss()
    print(loss)
    print(out)
    print(out.shape)
