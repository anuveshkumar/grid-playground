...
from lib.neus import singlescalegrid_mono
...
import cv2
import shutil
from lib.bg_model import calc_ray_bbox_intersection
...
def train(args, cfg, data_dict):
    ...
    shutil.copy('lib/neus/singlescalegrid_mono.py', os.path.join(cfg.basedir, cfg.expname, 'model.py'))
    ...
    model = singlescalegrid_mono.SingleScaleVolSDF(xyz_min, xyz_max, resolution=resolution, **model_kwargs)
    ...
    for global_step in trange(1 + start, 1 + cfg_train.N_iters):
        ...
        bbox_intersections, mask_intersect = calc_ray_bbox_intersection(rays_o, rays_d, xyz_min, xyz_max)
        near, far = bbox_intersections[..., 0:1], bbox_intersections[..., 1:2]
        render_kwargs.update({'near': near[mask_intersect], 'far': far[mask_intersect]})
        # if cfg.data.load2gpu_on_the_fly:
        target = target.to(device)[mask_intersect]
        rays_o = rays_o.to(device)[mask_intersect]
        rays_d = rays_d.to(device)[mask_intersect]
        viewdirs = viewdirs.to(device)[mask_intersect]
...
if __name__ == '__main__':
    ...
    if args.render_test or args.render_train or args.render_video or args.extract_mesh:
        ...
        model = utils.load_model(singlescalegrid_mono.SingleScaleVolSDF, ckpt_path).to(device)
