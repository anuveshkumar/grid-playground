...
from lib.nerf import testing_pref
...
def config_parser():
    ...
    parser.add_argument('--config', default='configs/nerf/lego.py', help='config file path')
...
def train(args, cfg, data_dict):
    ...
    model = model_classes[model_index](xyz_min, xyz_max, **model_kwargs)
    ...
    for global_step in trange(1 + start, 1 + cfg_train.N_iters):
        ...
        if global_step in cfg_train.pg_scale:
            resolution = int(model.resolution + 128 / len(cfg_train.pg_scale))
            model._set_grid_resolution(resolution=resolution)
            model.expose = int(model.expose + 128 / len(cfg_train.pg_scale))
        ...
        # losses['parseval_1'] = 1e-4 * model.density_encoder.parseval_loss()
        # losses['parseval_2'] = 1e-4 * model.feature_encoder.parseval_loss()
