...
from lib.neus import singlescalegrid_neus
...
def config_parser():
    ...
    parser.add_argument('--config', default='configs/mvs/chair.py', help='config file path')
...
def train(args, cfg, data_dict):
    ...
    shutil.copy('lib/neus/singlescalegrid_neus.py', os.path.join(cfg.basedir, cfg.expname, 'model.py'))
    ...
    model = singlescalegrid_neus.SingleScaleNeus(xyz_min, xyz_max, resolution=resolution, **model_kwargs)
    ...
    #     points = torch.cat([torch.FloatTensor(40000, 3).uniform_(-0.5, 0.5),
    #                         torch.FloatTensor(40000, 3).uniform_(model.xyz_min[0], model.xyz_max[0])]).cuda()
    #     sdf_gt = points.norm(dim=-1) - radius
    #     latents, normals = model.extract_latent(points)
    #     sdf_pred = latents[..., 0]
    ...
    # mesh.export(os.path.join(cfg.basedir, cfg.expname, 'pretrain.ply'.format(global_step)))
...
def get_cos_anneal_ratio(global_step, anneal_end=5000):
    if anneal_end == 0.0:
        return 1.0
    else:
...
if __name__ == '__main__':
    ...
    if args.render_test or args.render_train or args.render_video or args.extract_mesh:
        ...
        model = utils.load_model(singlescalegrid_neus.SingleScaleNeus, ckpt_path).to(device)
